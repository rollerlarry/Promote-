...
<div id="sub_banner" <?php if ( get_header_image() ) : ?>style="background:url(<?php header_image(); ?>)no-repeat center center /cover;" <?php endif; ?>>
 <h1>
	<?php
		/**
		 * Filter the promote author bio avatar size.
		 *
		 * @since promote
		 *
		 * @param int $size The avatar height and width size in pixels.
		 */
		$author_bio_avatar_size = apply_filters( 'promote_author_bio_avatar_size', 42 );

		echo get_avatar( get_the_author_meta( 'user_email' ), $author_bio_avatar_size );
	?>
   <?php esc_attr__('Posts by ', 'promote');?><?php echo get_the_author(); ?>
